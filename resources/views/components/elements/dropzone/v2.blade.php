...
Dropzone.autoDiscover = false;
  ...
  foto_upload_{{ $n }}.on("success",function(file,response_server){
    // console.log( file );
    var dt = response_server;
    var html_root = jQuery('a.dz-remove').parent();
    ...
    {
      alert(dt.error);
      // hapus nama file yang gagal
      html_root.eq( html_root.length -1 ).remove();
      return;
    }
    file.id_res_server = dt.id;
    console.log(response_server);
  });
  foto_upload_{{ $n }}.on("error",function(file,response_server){
    alert(response_server);
    console.log(response_server);
    // jQuery("a.dz-remove").parent().remove();
    var htmlroot = jQuery('a.dz-remove').parent();
    htmlroot.eq( htmlroot.length - 1).remove();
    // return false;
  });
    ...
    $.ajax({
        ...
        success: function(dt){
          ...
          if(dt.status)
          {
            ...
            foto_upload_{{ $n }}.emit("added", a);
            foto_upload_{{ $n }}.emit("complete", a);
            alert( dt.pesan );
          }
          console.log(dt);
